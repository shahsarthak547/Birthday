...
const StatCard = ({ icon: Icon, title, value, color, delay }) => (
  <motion.div
    ...
    className={`relative overflow-hidden glass-card flex flex-col items-center text-center group border-2 border-transparent hover:border-white/20 transition-all duration-300`}
  >
    ...
    <div className={`p-6 rounded-3xl ${color} mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-[0_0_30px_rgba(0,0,0,0.5)]`}>
      <Icon size={40} className="text-white drop-shadow-md" />
    </div>

    <h3 className="text-white/50 text-xs font-bold uppercase tracking-[0.2em] mb-3">{title}</h3>
    <p className="text-xl md:text-2xl font-black text-white drop-shadow-[0_2px_10px_rgba(255,255,255,0.4)] leading-tight">
      {value}
    </p>
  </motion.div>
);
